...
All files that reside in the root `docs/` directory.

If it references another documentation file, I also search that file.

For example, the `docs/UNIT-TESTS.md` file points to `tests/unit/README.md` for detailed guidance.
...
An update can be a new section (e.g., "DTO Validation Tests") or an addition to an existing section (e.g., adding a bullet point under "Code Constraints").
...
2. **Show full diff** to user:
   ...
   ```
3. **Ask for final approval**: If accepted, edit the file

4. **If rejected**: Ask "What needs to change?" and iterate
...
**Before I editing**, I should warn:
...
- ✅ I showed full diff to user before editing
- ✅ User confirmed approval before editing
...
All skills can be improved through this auto-learn process.

Every skill is located in `.agents/skills/[skill-name]/SKILL.md`.

### Key documentation

- `AGENTS.md` — Code style, naming, constraints
- All docs under `docs/` and pointers to other files (e.g., `docs/UNIT-TESTS.md` → `tests/unit/README.md`)
