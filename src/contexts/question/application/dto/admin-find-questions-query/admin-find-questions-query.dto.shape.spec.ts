...
describe("Admin Find Questions Query DTO Shape", () => {
  ...
  describe("ids", () => {
    ...
    it("should throw zod error when ids contains invalid values.", () => {
      ...
      expect(() => ADMIN_FIND_QUESTIONS_QUERY_DTO.parse(dtoWithInvalidIds)).toThrow(ZodError);
    });

    it("should pass validation when ids is not provided.", () => {
      const { ids: _, ...dtoWithoutIds } = validDto;

      expect(() => ADMIN_FIND_QUESTIONS_QUERY_DTO.parse(dtoWithoutIds)).not.toThrow();
    });

    it("should pass validation when ids is a single string value.", () => {
      const dtoWithSingleId = { ...validDto, ids: "60af924f4f1a2563f8e8b456" };

      const result = ADMIN_FIND_QUESTIONS_QUERY_DTO.parse(dtoWithSingleId);
    ...
    it("should throw zod error when ids is an empty array.", () => {
      ...
      expect(() => ADMIN_FIND_QUESTIONS_QUERY_DTO.parse(dtoWithEmptyIds)).toThrow(ZodError);
    });
    ...
    it("should throw zod error when ids is a comma-separated string of invalid values.", () => {
      ...
      expect(() => ADMIN_FIND_QUESTIONS_QUERY_DTO.parse(dtoWithCommaSeparatedInvalidIds)).toThrow(ZodError);
    });
    ...
    it("should throw zod error when ids contains duplicate values.", () => {
      ...
      expect(() => ADMIN_FIND_QUESTIONS_QUERY_DTO.parse(dtoWithDuplicateIds)).toThrow(ZodError);
    });
  });
