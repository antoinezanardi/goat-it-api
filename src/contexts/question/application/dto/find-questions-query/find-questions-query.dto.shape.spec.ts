...
describe("Find Questions Sort Query DTO Shape", () => {
  ...
  describe("theme-ids", () => {
    ...
    it("should have correct description on array when accessing the description.", () => {
      expect(FIND_QUESTIONS_QUERY_DTO.shape["theme-ids"].description).toBe("List of theme IDs to filter questions by (OR logic)");
    });

    it("should have correct description on array element when accessing the description.", () => {
      expect(FIND_QUESTIONS_QUERY_DTO.shape["theme-ids"].unwrap().element.description).toBe("Theme ID to filter questions by");
    });
  });
