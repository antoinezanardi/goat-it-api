...
Feature: Limit Questions as Admin
  ...
  Scenario: Limiting admin questions to 2 results
  ...
  Scenario: Returning all admin questions when limit is 0 (unlimited)
    Given the database is populated with questions fixture set with name "five-questions"
    When the admin retrieves all questions with the following query:
      | limit |
      | 0     |
  ...
  Scenario: Trying to limit admin questions with an invalid limit value
    Given the database is populated with questions fixture set with name "five-questions"
    When the admin retrieves all questions with the following query:
      | limit |
      | -1    |
  ...
  Scenario: Trying to limit admin questions with an invalid value type
    Given the database is populated with questions fixture set with name "five-questions"
    When the admin retrieves all questions with the following query:
      | limit |
      | abc   |
