...
Feature: Filter Questions as Admin
  ...
  Scenario: Filtering admin questions by adult content "false"
    ...
    Then the request should have succeeded with status code 200
    And the response should contain 3 admin questions
    And the response should contain an admin question among them with id "a1b2c3d4e5f6012345678901"
  ...
  Scenario: Filtering admin questions with invalid is-adult-content value
    Given the database is populated with questions fixture set with name "five-questions"
    When the admin retrieves all questions with the following query:
      | is-adult-content |
      | maybe            |
