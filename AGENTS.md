...
2) Risk-based rules and additional checks
...
3) Files agents can and cannot modify
...
5) Abort / save procedure if gates fail
...
6) Operational checklist before opening a PR
...
- Package manager: pnpm (pnpm@10.19.0 as declared)
- Node engine: Node.js >= 24.10.0 (see `package.json` `engines` field)
...
- `tests/unit/...` — Unit test outputs and helpers (mocks/factories paths available in Vitest aliases).
...
- Types/interfaces/enums/constants: use `<feature>.types.ts`, `<feature>.interface.ts`, `<feature>.enum.ts`, `<feature>.constants.ts` as appropriate.
...
1. Create a directory: `src/modules/<feature-name>/` (use `kebab-case` for folder names).
...
- Coverage: V8 provider; coverage directory `tests/unit/coverage` and coverage thresholds are strict (the project currently enforces `100%` in config).
...
- Run `pnpm run test:unit` and ensure all tests pass.
...
```bash
pnpm install
pnpm run prepare   # optional project-specific prepare steps
```
...
Notes and warnings
-----------------
- The project enforces strict coverage (all are 100%). Ensure any changes maintain coverage.
...
- Issues: https://github.com/antoinezanardi/goat-it-api/issues
